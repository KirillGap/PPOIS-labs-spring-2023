...
class POISlab2(MDApp):
    ...
    def get_rows(self):
        ...
        for item in data_temp:
            data["items"].append({
                "pet_name": 0 if item[0] == "" else item[0],
                "date_of_birth": item[1],
                "date_of_last_admission": item[2],
                "FIO_vet":item[3],
                "diagnosis": item[4]
            })
        return data
    ...
    def read_file(self):
        ...
        if os.path.isfile(file_name["File Name"].text):
            ...
            for item in data_base["items"]:
                self.table.add_row((item["pet_name"], item["date_of_birth"],
                                          item["date_of_last_admission"], item["FIO_vet"],
                                          item["diagnosis"]))
            json_file.close()
    ...
    def search_and_output_rows(self):
        ...
        for field in filled_fields:
            if filled_fields[field].text:
                ...
                if section_for_search == "pet_name":
        ...
        return MDAnchorLayout(
            MDDataTable(
                ...
                column_data=[
                    ("Имя питомца", dp(60)),
                    ("Дата рождения", dp(45)),
                    ("Дата последнего приёма", dp(60)),
                    ("ФИО ветеринара", dp(60)),
                    ("Диагноз", dp(60)),
                ],
                row_data = new_data
            )
    ...
    def search_and_delete_rows(self):
        ...
        for item in new_data["items"]:
            self.table.add_row((item["pet_name"], item["date_of_birth"],
                                      item["date_of_last_admission"], item["FIO_vet"], item["diagnosis"]))
        self.close_string_filtering_dialog()
    ...
    def build(self):
        ...
        self.table = MDDataTable(
                ...
                column_data=[
                    ("Имя питомца", dp(45)),
                    ("Дата рождения", dp(45)),
                    ("Дата последнего приёма", dp(45)),
                    ("ФИО ветеринара", dp(45)),
                    ("Диагноз",dp(45)),
                ],
    ...
    def add_row(self):
        ...
        self.table.add_row((item["pet_name"].text, item["date_of_birth"].text,
                                  item["date_of_last_admission"].text, item["FIO_vet"].text,
                                  item["diagnosis"].text))
        self.dialog_for_adding.dismiss()
    ...
    def show_string_adding_dialog(self):
        ...
        content_cls = MDBoxLayout(
            ...
            MDTextField(
                id = 'date_of_last_admission',
                hint_text="Дата последнего приёма",
                font_size='20',
                max_text_length = 50,
            ),
            MDTextField(
                id = 'FIO_vet',
                hint_text="ФИО ветеринара",
                font_size='20',
                max_text_length = 50,
            ),
            MDTextField(
